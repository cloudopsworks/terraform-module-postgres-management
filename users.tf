...
locals {
  user_role_names = merge(
    { for key, role in postgresql_role.user : key => role.name },
    { for key, role in postgresql_role.user_rotated : key => role.name }
  )
}
...
resource "postgresql_role" "user" {
  for_each = {
    for key, user in var.users : key => user if var.rotation_lambda_name == ""
  }
  ...
  roles = try(each.value.grant, "") == "owner" ? (
    ...
    [local.owner_role_names[each.value.db_ref]] : []
  ) : []
}

resource "postgresql_role" "user_rotated" {
  for_each = {
    for key, user in var.users : key => user if var.rotation_lambda_name != ""
  }
  name = each.value.name
